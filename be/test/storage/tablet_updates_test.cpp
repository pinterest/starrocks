...
TEST_F(TabletUpdatesTest, load_snapshot_primary_with_gin_index) {
    ...
    auto add_version = [&](TabletSharedPtr& tablet, int64_t version) {
        ...
        ASSERT_TRUE(tablet->rowset_commit(version, create_rowset(tablet, keys)).ok());
    };
    ...
    ASSERT_OK(
            SnapshotManager::instance()->convert_rowset_ids(snapshot_dir, src_tablet->tablet_id(), src_tablet->schema_hash()));
    snapshot_dirs.clear();
    ...
    ASSERT_OK(SnapshotManager::instance()->link_inverted_index_directories(
            *snapshot_meta, dest_tablet->tablet_schema(), snapshot_dir, dest_tablet->schema_hash_path(),
            &linked_index_dirs));
    ASSERT_FALSE(linked_index_dirs.empty());

    for (const auto& file : snapshot_files) {
        const std::string src = snapshot_dir + "/" + file;
        const std::string dst = dest_tablet->schema_hash_path() + "/" + file;
        ASSERT_OK(FileSystem::Default()->link_file(src, dst));
    }

    ASSERT_OK(dest_tablet->updates()->load_snapshot(*snapshot_meta));
    ...
    ASSERT_TRUE(std::any_of(dest_dirs_after.begin(), dest_dirs_after.end(),
                            [](const std::string& dir) { return dir.ends_with(".ivt"); }));
}
...
TEST_F(TabletUpdatesTest, link_primary_snapshot_gin_indexes_is_idempotent) {
    ...
    ASSERT_OK(tablet->rowset_commit(2, create_rowset(tablet, {21, 22, 23})));
    ...
    ASSERT_OK(SnapshotManager::instance()->convert_rowset_ids(snapshot_dir, tablet->tablet_id(), tablet->schema_hash()));
    auto snapshot_meta = SnapshotManager::instance()->parse_snapshot_meta(snapshot_dir + "/meta");
    ASSERT_OK(snapshot_meta.status());
    ASSERT_OK(SnapshotManager::instance()->assign_new_rowset_id(&*snapshot_meta, snapshot_dir, tablet->tablet_schema()));
    ...
    ASSERT_OK(fs::list_dirs_files(destination_index_dir, nullptr, &existing_index_files));
    ...
    const std::string missing_index_file = destination_index_dir + "/" + index_file_name;
    ...
    const std::string unexpected_file = destination_index_dir + "/unexpected";
    ...
    ASSERT_TRUE(st.is_corruption()) << st;
    ASSERT_TRUE(fs::path_exist(destination_index_dir));
